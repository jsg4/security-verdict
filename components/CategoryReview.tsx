...
export default function CategoryReview({
  ...
  return (
    <div className={`mb-8 ${isTopPick ? "ring-2 ring-[var(--gold)]" : ""}`} id={product.slug}>
      ...
      <div
        className={`border border-[var(--gray-200)] bg-white shadow-sm ${isTopPick ? "rounded-b-[var(--radius-lg)]" : "rounded-[var(--radius-lg)]"} overflow-hidden`}
      >
        <a
          href={product.url}
          target="_blank"
          rel="noopener noreferrer nofollow"
          className="block hover:shadow-lg transition-all no-underline text-inherit"
        >
          {/* Header */}
              ...
              <div className="flex items-center gap-4 flex-1 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {product.bestFor && <BestForBadge bestFor={product.bestFor} />}
                  </div>
                  ...
                  {product.deal && <div className="mt-2"><DealBadge deal={product.deal} /></div>}
                </div>
              </div>
        ...
        {/* ExpertTake and ClickToCall live outside the <a> to avoid nested interactive elements */}
        <div className="px-4 sm:px-6 pb-6">
          {product.phone && (
            <div className="mt-4">
              <ClickToCall phone={product.phone} productName={product.name} variant="prominent" />
            </div>
          )}
          {product.expertTake && (
            <ExpertTake content={product.expertTake} authorName={authorName} authorPhoto={authorPhoto} />
          )}
        </div>
      </div>
    </div>
